...
def read_aluno_por_nome(nome_aluno: str, db: Session = Depends(get_db)):
    ...
    """
    db_alunos = db.query(ModelAluno).filter(ModelAluno.nome.ilike(
        f"%{nome_aluno}%")).all()  # ilike para case-insensitive
...
def read_aluno_por_email(email_aluno: str, db: Session = Depends(get_db)):
    ...
    """
    db_aluno = db.query(ModelAluno).filter(
        ModelAluno.email == email_aluno).first()

    if db_aluno is None:
        raise HTTPException(
            status_code=404, detail="Nenhum aluno encontrado com esse email")
